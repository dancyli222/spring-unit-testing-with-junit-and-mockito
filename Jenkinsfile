        ...
        stage('Code analysis with SonarQube'){
            steps{
                ...
                withSonarQubeEnv('sonar'){
                    sh 'mvn clean verify org.sonarsource.scanner.maven:sonar-maven-plugin:3.2:sonar -Dsonar.projectKey=unit-testing -Dsonar.login=admin -Dsonar.password=adminadmin'
                }
            }
        }
        stage('Unit Test'){
